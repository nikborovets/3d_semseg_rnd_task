...
def downsample_point_cloud(pcd, method="voxel", voxel_size=0.025, target_points=100000):
    ...
    if method == "voxel":
        ...
        print(
            f"After voxel downsampling (voxel_size={voxel_size}): {len(pcd_downsampled.points)}"
        )
        ...
        if len(pcd.points) > target_points:
            ...
            print(
                f"After random downsampling (target={target_points}): {len(pcd_downsampled.points)}"
            )
        else:
            pcd_downsampled = pcd
            print(
                f"No downsampling needed (points={len(pcd.points)} <= target={target_points})"
            )
        ...
        if normals is not None:
            pcd_downsampled.normals = o3d.utility.Vector3dVector(
                normals[unique_indices]
            )

        print(
            f"After grid downsampling (grid_size={voxel_size}): {len(pcd_downsampled.points)}"
        )

    else:
        raise ValueError(
            f"Unknown downsampling method: {method}. Use 'voxel', 'random' or 'grid'"
        )
...
def get_kpconv_format_info():
    ...
    return {
        ...
        "data_types": {"points": "np.float32", "features": "np.float32"},
...
def downsample_and_save_ply(input_pcd_path, voxel_size=0.03):
    ...
    output_ply_path = os.path.join(
        os.path.dirname(input_pcd_path), f"{base_name}_voxel_{voxel_size}.ply"
    )
    ...
    if not pcd_downsampled.has_colors():
        ...
        pcd_downsampled.colors = o3d.utility.Vector3dVector(
            np.ones((num_points, 3)) * 0.5
        )
    ...
    if success:
        ...
        print(
            f"Original points: {len(pcd.points)}, Downsampled points: {len(pcd_downsampled.points)}"
        )
    else:
