...
import numpy as np
from demo.pcd_preprocessor import load_and_preprocess_pcd
    ...
    if array.size > 0:
        ...
        if key in ['coord', 'color', 'normal'] and array.ndim == 2:
            ...
            for i in range(array.shape[1]):
                axis_data = array[:, i]
                print(f"  Ось {i}: min={np.min(axis_data):.6f}, max={np.max(axis_data):.6f}, mean={np.mean(axis_data):.6f}")
...
def analyze_point_dict(point_dict):
    ...
    """
    print(f"\n{'#'*80}")
    print(f"ДЕТАЛЬНЫЙ АНАЛИЗ СЛОВАРЯ POINT")
    print(f"{'#'*80}")
    ...
    print(f"{'#'*80}")
    total_memory = sum(arr.nbytes for arr in point_dict.values() if isinstance(arr, np.ndarray)) / (1024 * 1024)
    print(f"Общий объем данных: {total_memory:.2f} MB")
    print(f"Количество точек: {len(point_dict['coord']) if 'coord' in point_dict else 'N/A'}")
    print(f"{'#'*80}")
    ...
    for key, value in point_dict.items():
        if isinstance(value, np.ndarray):
            summary[key] = {
                ...
                'unique_count': len(np.unique(value)) if np.issubdtype(value.dtype, np.integer) else None
            }
        else:
            summary[key] = {
                'type': str(type(value)),
