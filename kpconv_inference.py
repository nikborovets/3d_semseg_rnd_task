...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="KPConv Semantic Segmentation Inference"
    )
    parser.add_argument(
...
class KPConvInferencer:
    ...
    def _load_model(self):
        ...
        self.model = KPFCNN(
            self.config, self.dataset.label_values, self.dataset.ignored_labels
        )
    ...
    def predict(self, points, features, chunk_size=30000):
        ...
        if num_points > chunk_size:
            ...
            for i in range(0, num_points, chunk_size):
                ...
                chunk_pred, chunk_probs, chunk_time = self._predict_chunk(
                    chunk_points, chunk_features
                )
...
def main():
    ...
    output_filename = (
        f"{input_filename}_KPConv_{model_name}_"
    ...
    print(
        f"Parameters: downsampling={args.downsampling_method}, voxel_size={args.voxel_size}m, chunk_size={args.chunk_size}, device={args.device}"
    )
    ...
    try:
        ...
        try:
            ...
            predictions, probs = inferencer.predict(
                points, features, chunk_size=args.chunk_size
            )
        ...
        for class_id, count in zip(unique, counts):
            class_name = inferencer.dataset.label_to_names.get(
